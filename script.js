(() => {
  ...
  async function rememberPasswordStore(pw) {
    ...
    localStorage.setItem(
      REMEMBER_KEY,
      JSON.stringify({ iv: bufToB64(iv), data: bufToB64(enc) })
    );
  }
  ...
  function syncSettingsFromUI() {
    ...
    s.defaultChoices = clampInt(defaultChoices.value, 4, 6, 4);
    s.model = (model.value || "gpt-4.1-mini").trim() || "gpt-4.1-mini";
    s.endpoint = (
      endpoint.value || "https://api.openai.com/v1/responses"
    ).trim();
  ...
  /* =========================
     DIFFICULTY SPECS (BACKEND)
  ========================= */
  function difficultySpec(level) {
    if (level === "easy") {
      return [
        "EASY:",
        "- Direct recall from one sentence/phrase.",
        "- No tricky distractors.",
        "- Answers should be obvious if you read the text once.",
    ...
    if (level === "hard") {
      return [
        "HARD:",
        "- Require inference or connecting multiple parts of the text.",
        "- Distractors must be plausible but clearly wrong based on text.",
        "- Avoid ambiguity: exactly one correct choice.",
        "- Prefer “why/how/implication” type questions grounded in the text."
      ].join("\n");
    }
    return [
      "MEDIUM:",
      "- Tests understanding, paraphrase, cause/effect within the text.",
      "- Distractors plausible but not deceptive.",
      "- Sometimes require combining 2 nearby ideas.",
  ...
  function buildGenerateUserInput({ text, count, diff, title, choicesCount }) {
    // MUST contain the word "json" to satisfy json_object guardrail
    const t = (title || "").trim();
    return [
      "Output format: json",
  ...
  function buildAddMoreUserInput({
    existingQuestions,
    addCount,
    diff,
    choicesCount,
  ...
  async function callOpenAI({ system, user }) {
    ...
    if (!s.apiKey)
      throw new Error("API key is blank. Paste it in Settings and Save.");
  ...
  function normalizeQuestion(q, choicesCount) {
    ...
    const choices = Array.isArray(q?.choices)
      ? q.choices.map((x) => String(x).trim()).filter(Boolean)
      : [];
    let ai = Number.isFinite(q?.answer_index)
      ? q.answer_index
    ...
    if (choices.length !== choicesCount)
      throw new Error(
        `Bad model output: choices must be exactly ${choicesCount}.`
      );
  ...
  function normalizeQuiz(raw, { diff, title, sourceText, choicesCount }) {
    ...
    if (!Array.isArray(questionsRaw) || questionsRaw.length === 0) {
      throw new Error("Bad model output: missing questions array.");
    }
    return {
      id: uid(),
  ...
  function computeGrade(quiz) {
    ...
    const accuracyAnswered = answered
  ...
  function renderResultsPanel(quiz) {
    ...
    const g = quiz.grade;
    const lines = [];
    lines.push(`<div><strong>Submitted.</strong></div>`);
    lines.push(
      `<div class="muted">Correct: ${g.correct}/${g.total} • Accuracy (total): ${g.accuracyTotal}% • Accuracy (answered): ${g.accuracyAnswered}%</div>`
    );
    lines.push(`<ul>`);
    for (let i = 0; i < quiz.questions.length; i++) {
      ...
      lines.push(
        `<li>${tag} <strong>${
          i + 1
        }</strong> • your: ${ua} • correct: ${ca} — ${escapeHtml(q.q)}</li>`
      );
    }
    lines.push(`</ul>`);
  ...
  function renderQuizList() {
    ...
    setStatus(
      libraryStatus,
      quizzes.length ? `${quizzes.length} quiz(es)` : "No quizzes yet."
    );
    ...
    for (const q of sorted) {
      const answered = (q.questions || []).filter(
        (x) => x.user_answer_index !== null && x.user_answer_index !== undefined
      ).length;
      ...
      row.innerHTML = `
        ...
        <div class="item__meta">${
          q.questions.length
        } Q • answered ${answered} • ${escapeHtml(q.difficulty)} • ${
        q.submitted ? "submitted" : "not submitted"
      }</div>
  ...
  function renderPlayer() {
    ...
    playerMeta.textContent = `${quiz.questions.length} questions • ${
      quiz.difficulty
    } • ${quiz.submitted ? "submitted" : "in progress"}`;
    ...
    q.choices.forEach((c, i) => {
      ...
      btn.addEventListener("click", () => {
        if (quiz.submitted) return; // locked after submit
    ...
    const answered =
      q.user_answer_index !== null && q.user_answer_index !== undefined;
    if (!answered) {
      qExplain.textContent = "Pick an answer to reveal the explanation.";
    } else if (quiz.submitted) {
      const ok = q.user_answer_index === q.answer_index;
      qExplain.textContent =
        (ok ? "✅ Correct. " : "❌ Wrong. ") +
        (q.explanation || "(No explanation provided.)");
    } else {
      qExplain.textContent = q.explanation || "(No explanation provided.)";
    }
    ...
    quiz.questions.forEach((qq, i) => {
      ...
      const answered2 =
        qq.user_answer_index !== null && qq.user_answer_index !== undefined;

      b.className =
        "qdot" + (i === idx ? " active" : "") + (answered2 ? " answered" : "");

      if (quiz.submitted && grade?.per?.[i]) {
        b.classList.add(
          grade.per[i].isCorrect ? "good" : grade.per[i].isAnswered ? "bad" : ""
        );
      }
  ...
  async function generateQuiz() {
    ...
    try {
      ...
      const choicesCount = clampInt(defaultChoices.value, 4, 6, 4);
  ...
  async function addMoreQuestions() {
    ...
    if (quiz.submitted) {
      setStatus(
        playerStatus,
        "Quiz already submitted. Copy it if you want a fresh run.",
        true
      );
      return;
    }
    ...
    try {
      ...
      if (!qs.length)
        throw new Error("Bad model output: no questions returned.");
  ...
  function setLockMode() {
    if (!hasVault()) {
      ...
      lockHint.textContent =
        "First run. Create a master password to encrypt everything stored locally.";
      confirmWrap.hidden = false;
    } else {
      App.mode = "unlock";
      lockTitle.textContent = "Unlock";
      lockHint.textContent =
        "Enter your master password to decrypt local data.";
  ...
  async function unlockOrSetup() {
    ...
    try {
      ...
      if (App.mode === "setup") {
        ...
        if (pw !== pw2)
          return setStatus(lockStatus, "Passwords do not match.", true);

        App.vault = defaultVault();
        App.password = pw;
        await saveVaultNow();
      } else {
        const env = JSON.parse(localStorage.getItem(VAULT_KEY));
        App.vault = await decryptVault(env, pw);
        App.password = pw;

        // migrate missing settings
        if (!App.vault.settings) App.vault.settings = defaultVault().settings;
        if (
          App.vault.settings.temperature === undefined ||
          App.vault.settings.temperature === null
        )
          App.vault.settings.temperature = 0.7;
        if (!App.vault.settings.model)
          App.vault.settings.model = "gpt-4.1-mini";
        if (!App.vault.settings.defaultChoices)
          App.vault.settings.defaultChoices = 4;
      }
      ...
      // auto-load latest quiz
      if (!App.currentQuizId && App.vault.quizzes.length) {
        const sorted = App.vault.quizzes
          .slice()
          .sort((a, b) =>
      ...
      setStatus(lockStatus, "Wrong password (or corrupted vault).", true);
    } finally {
  ...
  async function resetAll() {
    const ok = confirm(
      "Wipe EVERYTHING stored locally for this app? This cannot be undone."
    );
    if (!ok) return;
  ...
  nextBtn.addEventListener("click", () => {
    const quiz = getActiveQuiz();
    if (!quiz) return;
    quiz.currentIndex = Math.min(
      quiz.questions.length - 1,
      (quiz.currentIndex || 0) + 1
    );
    quiz.updated_at = nowISO();
    scheduleSave();
    renderPlayer();
  ...
  // live sync settings (prevents blank api key)
  ...
  defaultChoices.addEventListener("change", () => {
    if (App.vault) {
      syncSettingsFromUI();
      scheduleSave();
    }
  ...
  ========================= */
  (async () => {
    // init model options immediately (for fresh installs)
    model.innerHTML = "";
    for (const opt of MODEL_OPTIONS) {
      const o = document.createElement("option");
      o.value = opt.id;
    ...
    // auto unlock if remembered password exists
    const remembered = await rememberPasswordLoad();
    if (remembered && hasVault()) {
      masterPassword.value = remembered;
